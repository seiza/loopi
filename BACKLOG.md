# Loopi Backlog

This file contains candidates for future resource pages on Loopi.

## Robot
- [x] **Bee-Bot**: A programmable floor robot perfect for introducing control, directional language, and programming to young children.
- [x] **Thymio**: An open-source educational robot that can be programmed with blocks or text.

## Débranché (sans ordinateur)
- [x] **CS Unplugged**: A collection of free teaching material that teaches Computer Science through engaging games and puzzles.
- [x] **Hello Ruby**: A book series and website that makes coding and logic accessible through stories and activities.
- [x] **Graph Paper Programming**: Learn how to guide a "robot" (your friend!) to draw a picture on graph paper using only a small set of symbols.

## Numérique (avec écran)
- [x] **Tynker**: An educational programming platform designed to help children learn to code through games.
- [x] **Code.org**: A nonprofit dedicated to expanding access to computer science in schools and increasing participation by young women and students from other underrepresented groups.
- [x] **Blockly**: A library that adds a visual code editor to web and mobile apps.
- [x] **Lightbot**: An educational game that uses logic puzzles fundamental to coding, like loops and procedures.

## Jeux de société
- [x] **Turing Tumble**: A mechanical computer powered by marbles to solve logic puzzles.
- [x] **Code Master**: A programming logic game that uses an avatar to travel through a map to reach a portal.
- [x] **On the Dot**: A pattern-matching game that challenges spatial logic.
- [x] **Potato Pirates**: A card game that teaches programming concepts like loops, conditionals, and functions.

## Dictionnaire (écosystème)
- [x] **Variable**: A way to store data that can be used and changed throughout a program.
- [x] **Function**: A reusable block of code that performs a specific task.
- [x] **Conditional (If/Then)**: A basic logic structure that tells a program to perform different actions depending on whether a condition is true or false.
- [x] **Loop**: A sequence of instructions that is continually repeated until a certain condition is reached.
